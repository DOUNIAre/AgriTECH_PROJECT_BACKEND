...
if (!email || !password || !name) { 
  return res.status(400).json({
    ...
    // Validate role
    if (role && !["farmer", "producer", "client"].includes(role)) {
      return res.status(400).json({
        success: false,
        error: { code: "INVALID_ROLE", message: "Role must be 'farmer' or 'producer' or 'client'" },
      });
    }
...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    if (preferredLanguage && !["ar", "fr", "en"].includes(preferredLanguage)) {
      return res.status(400).json({
        success: false,
    ...
    res.json({
      ...
      data: {
        user: {
          ...
          bio: updatedUser.bio,
          scorePoints: updatedUser.scorePoints,
